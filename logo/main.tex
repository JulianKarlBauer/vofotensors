...
}
{
    ...
    % \draw[-, \RandomColor]
    % (xyz spherical cs:radius=\myradius, angle=\i, latitude=\j)
    %  --
    % (xyz spherical cs:radius=-\myradius, angle=\i, latitude=\j);

    % \draw[-, \RandomColor, line width=0.4mm]
    % (xyz cs:x={cos(\i)*sin(\j)*\myradius}, y={sin(\i)*sin(\j)*\myradius}, z={cos(\j)*\myradius*\myfactor})
    %  --
    % (xyz cs:x={cos(\i)*sin(\j)*-\myradius}, y={sin(\i)*sin(\j)*-\myradius}, z={cos(\j)*-\myradius*\myfactor});
